...
class Executions():
	...
	def __init__(self, conn, flower, results_path, make_mosaic_script, gif_algo, gif_script):
		self.conn = conn
		self.flower = flower
		self.executions = []
		self.results_path = results_path
		self.make_mosaic_script = make_mosaic_script
		self.gif_algo = gif_algo
	...
	def load_enqueued_executing(self):
		...
		for each_execution in dao_execution.get_enqueued_executing():
			execution = Execution(each_execution, conn=self.conn, flower=self.flower, results_path=self.results_path, make_mosaic_script=self.make_mosaic_script, gif_algo=self.gif_algo, gif_script=self.gif_script)
